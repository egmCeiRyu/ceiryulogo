// ====================== 8TH WALL IMAGE TARGET + GLB ANCORA ======================

(() => {
  "use strict";

  // 1. Configura o Image Target
  ...
  if (window.XR8) {
    ...
    window.addEventListener("xrloaded", configureImageTarget);
  }

  // 2. Pipeline Module - Mostrar e ancorar o modelo
  const imageTargetModule = {
    name: "image-target-anchoring",
    listeners: [
      {
        ...
        process: ({ detail }) => {
          ...
          scene.traverse((child) => {
            if (child.name === "Untitled.glb") {
              model = child;
            }
          });

          if (!model) {
            console.warn("❌ Modelo Untitled.glb não encontrado");
            return;
          }
  ...
  // 3. Esconde o modelo no início
  const hideModelModule = {
    name: "hide-model-on-start",
    onStart: () => {
      const { scene } = XR8.Threejs.xrScene();
      let found = false;
      ...
      scene.traverse((child) => {
        if (child.name === "Untitled.glb") {
          child.visible = false;
          found = true;
          console.log("🙈 Modelo escondido - aguardando marker...");
        }
      });

      if (!found) console.warn("⚠️ Modelo 'Untitled.glb' não encontrado na cena");
    }
  };

  // 4. Adiciona os módulos
  const addPipelineModules = () => {
    XR8.addCameraPipelineModule(imageTargetModule);
    XR8.addCameraPipelineModule(hideModelModule);
  };

  if (window.XR8) {
    addPipelineModules();
  } else {
    window.addEventListener("xrloaded", addPipelineModules);
  }

  // 5. Inicializa a cena ECS
