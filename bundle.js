(() => {
  ...
  (() => {
    ...
    // Módulo para esconder o modelo no início
    const hideModule = {
      name: "hide-on-start",
      onStart: () => {
    ...
    // Módulo para detectar marker e ancorar o personagem (só posição)
    const anchorModule = {
      ...
      listeners: [{
        ...
        process: ({ detail }) => {
          ...
          model.visible = true;
          console.log("✅ Marker encontrado! Personagem ancorado (sem rotação)");
        }
      }]
    };
    ...
    if (window.XR8) addModules();
    else window.addEventListener("xrloaded", addModules);
  })();
  // ====================== FIM DO CÓDIGO ADICIONADO ======================

  t(574);
  window.ecs.registerComponent({ name: "example-component", add: () => { console.log("Component attached."); } });
